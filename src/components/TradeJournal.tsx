...
import {
  Filter, X, Save, Eye, Star, Camera, CheckCircle2,
  Calendar, RefreshCw, AlertTriangle, ShieldCheck,
  Target, TrendingUp, AlertOctagon, Activity, Lock
} from "lucide-react";
...
interface Trade {
  ...
  narrative?: string; trade_class?: string;
}
...
const COMPLIANCE_RULES = [
  "HTF Bias/Trend Confluence?", "Approved Entry Model?",
  "Optimal Location (Premium/Discount)?", "Minimum R:R Threshold Met?",
  "Cleared Macro News Window?", "Psychological State Stable?"
];

const TRADE_CLASSES = [
  { id: "A+", label: "A+ Setup (Perfect)", color: "#16a34a" },
  { id: "B", label: "B Setup (Standard)", color: "#2563eb" },
  { id: "GOOD_LOSS", label: "Good Loss (Systematic)", color: "#ea580c" },
  { id: "BAD_WIN", label: "Bad Win (Luck/Variance)", color: "#db2777" },
  { id: "BAD_LOSS", label: "Bad Loss (System Violation)", color: "#dc2626" }
];
...
export default function TradeJournal({ accountId }: { accountId: number }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const safeItems = compliance?.items || {};
  ...
  const handleComplianceCheck = (rule: string) => {
    setCompliance(prev => ({ ...prev, items: { ...(prev.items || {}), [rule]: !prev.items?.[rule] } }));
  };
  ...
  const handleWebImageUpload = () => {
    ...
    input.onchange = (e: any) => {
      ...
      Promise.all(promises).then(base64Images => {
        setResultImages(prev => [...prev, ...base64Images]);
      });
    };
    input.click();
  };
  ...
  const stats = useMemo(() => {
    ...
    const total = trades.filter(t => t.status === 'CLOSED' || t.pnl !== 0).length;
  ...
  return (
    <div style={{ height: 'calc(100vh - 80px)', display: 'flex', flexDirection: 'column', gap: '20px', fontFamily: 'Segoe UI' }}>
      ...
      <div style={{ display: 'flex', gap: '15px' }}>
        <StatCard title="Net PnL" value={`${stats.pnl > 0 ? '+' : ''}${stats.pnl.toFixed(2)}$`} color={stats.pnl >= 0 ? '#16a34a' : '#dc2626'} sub="Realized Valuation" />
        <StatCard title="Win Rate" value={`${stats.wr.toFixed(1)}%`} color="#2563eb" sub={`${stats.wins} / ${stats.total} Settled`} />
        <StatCard title="Total Interactions" value={trades.length} color="#64748b" sub="Including Omissions" />
        ...
        <div style={{ background: 'white', padding: '10px', borderRadius: '8px', border: '1px solid #e2e8f0', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: '5px', minWidth: '350px' }}>
          <div style={{ fontSize: '10px', fontWeight: 'bold', color: '#94a3b8', display: 'flex', gap: '5px', alignItems: 'center' }}><Filter size={10} /> SEARCH FILTERS</div>
          <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
            ...
            <select value={filterPair} onChange={e => setFilterPair(e.target.value)} style={{ padding: '4px', border: '1px solid #e2e8f0', borderRadius: '4px', fontSize: '11px', outline: 'none', maxWidth: '80px' }}>
              <option value="">All Assets</option>{uniquePairs.map(p => <option key={p} value={p}>{p}</option>)}
            </select>
            ...
            <select value={filterOutcome} onChange={e => setFilterOutcome(e.target.value)} style={{ padding: '4px', border: '1px solid #e2e8f0', borderRadius: '4px', fontSize: '11px', outline: 'none' }}>
              <option value="ALL">All States</option><option value="WIN">Yield</option><option value="LOSS">Deficit</option><option value="MISSED">Omission</option>
            </select>
            <button onClick={loadData} style={{ border: 'none', background: '#f1f5f9', borderRadius: '4px', cursor: 'pointer', padding: '4px', color: '#64748b' }} title="Fetch Database"><RefreshCw size={14} /></button>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {trades.map(t => {
            const review = safeJSONParse(t.review_data, {});
            const tradeClass = t.trade_class || "-";
            const mistake = (review.mistakes && review.mistakes[0]) || "-";
            ...
            return (
              <div key={t.uuid} style={{ display: 'grid', gridTemplateColumns: '1fr 0.5fr 0.5fr 0.8fr 0.8fr 1fr 1fr 1.5fr 80px', padding: '12px 20px', borderBottom: '1px solid #f1f5f9', fontSize: '13px', alignItems: 'center' }}>
                <div><div style={{ fontWeight: 'bold', color: '#1e293b' }}>{t.pair}</div><div style={{ fontSize: '10px', color: '#94a3b8' }}>{new Date(t.created_at * 1000).toLocaleString()}</div></div>
                <div><span style={{ padding: '2px 6px', borderRadius: '4px', fontSize: '10px', fontWeight: 'bold', background: t.direction === 'BUY' ? '#dcfce7' : '#fee2e2', color: t.direction === 'BUY' ? '#166534' : '#991b1b' }}>{t.direction === 'BUY' ? 'LONG' : 'SHORT'}</span></div>
                ...
                <div style={{ fontWeight: 'bold', color: t.pnl >= 0 ? '#16a34a' : '#dc2626' }}>{t.pnl > 0 ? '+' : ''}${t.pnl.toFixed(2)}</div>
                <div>
            ...
            <div style={{ background: 'white', padding: '15px 25px', borderBottom: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', gap: '20px', alignItems: 'center' }}>
                <h2 style={{ margin: 0, fontSize: '20px', color: '#0f172a', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  {selectedTrade.pair}
                  <span style={{ fontSize: '12px', padding: '4px 10px', borderRadius: '20px', background: selectedTrade.direction === 'BUY' ? '#dcfce7' : '#fee2e2', color: selectedTrade.direction === 'BUY' ? '#166534' : '#991b1b' }}>{selectedTrade.direction === 'BUY' ? 'LONG' : 'SHORT'}</span>
                </h2>
                <div style={{ display: 'flex', gap: '15px', fontSize: '13px', alignItems: 'center' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '5px', background: editPnL >= 0 ? '#f0fdf4' : '#fef2f2', padding: '4px 8px', borderRadius: '4px', border: editPnL >= 0 ? '1px solid #bbf7d0' : '1px solid #fecaca' }}>
                    <span style={{ fontSize: '11px', fontWeight: 'bold', color: editPnL >= 0 ? '#16a34a' : '#dc2626' }}>DELTA:</span>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={handleSave} style={{ background: '#2563eb', color: 'white', border: 'none', padding: '8px 20px', borderRadius: '8px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}><Save size={18} /> COMMIT DIAGNOSIS</button>
            ...
            <div style={{ flex: 1, padding: '20px', overflowY: 'auto', display: 'grid', gridTemplateColumns: '300px 1fr 320px', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <div style={{ background: 'white', padding: '15px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                  <h3 style={{ marginTop: 0, fontSize: '14px', color: '#1e293b', display: 'flex', alignItems: 'center', gap: '8px' }}><ShieldCheck size={18} /> SYSTEM ADHERENCE</h3>
                  <div style={{ marginBottom: '15px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                    {(() => {
                      const safeItems = compliance?.items || {};
                      const score = Math.round((Object.values(safeItems).filter(Boolean).length / COMPLIANCE_RULES.length) * 100);
                      return (
                        <div style={{ fontSize: '32px', fontWeight: 900, color: score >= 80 ? '#16a34a' : (score >= 50 ? '#ca8a04' : '#dc2626') }}>
                          {score}%
                        </div>
                      )
                    })()}
                    <div style={{ fontSize: '12px', color: '#64748b', lineHeight: '1.2' }}>System<br />Compliance</div>
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {COMPLIANCE_RULES.map(rule => (
                      <label key={rule} style={{ display: 'flex', gap: '10px', fontSize: '12px', cursor: 'pointer', alignItems: 'center', color: '#334155' }}>
                        <input type="checkbox" checked={compliance?.items?.[rule] || false} onChange={() => handleComplianceCheck(rule)} style={{ accentColor: '#2563eb' }} />
                        {rule}
                      </label>
                    ))}
                  </div>
                ...
                <div style={{ background: 'white', padding: '15px', borderRadius: '12px', border: '1px solid #e2e8f0', flex: 1 }}>
                  <h3 style={{ marginTop: 0, fontSize: '14px', color: '#1e293b', display: 'flex', alignItems: 'center', gap: '8px' }}><Target size={18} /> CLASSIFICATION</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {TRADE_CLASSES.map(cls => (
                      <button key={cls.id} onClick={() => setTradeClass(cls.id)}
                        style={{
                          padding: '8px', borderRadius: '6px', border: '1px solid', textAlign: 'left', cursor: 'pointer', fontSize: '12px', fontWeight: 'bold',
                          borderColor: tradeClass === cls.id ? cls.color : '#e2e8f0',
                          background: tradeClass === cls.id ? `${cls.color}10` : 'white',
                          color: tradeClass === cls.id ? cls.color : '#64748b'
                        }}>
                        {cls.label}
                      </button>
                    ))}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <div style={{ background: 'white', padding: '15px', borderRadius: '12px', border: '1px solid #e2e8f0', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <h3 style={{ marginTop: 0, fontSize: '14px', color: '#1e293b', display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}><Camera size={18} /> EVIDENCE FORENSICS</h3>
                  
                  {/* PLAN */}
                  <div style={{ marginBottom: '15px' }}>
                    <div style={{ fontSize: '11px', fontWeight: 'bold', color: '#64748b', marginBottom: '5px' }}>INITIAL HYPOTHESIS</div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '8px' }}>
                      {safeJSONParse(selectedTrade.images, []).map((path: string, i: number) => (
                      ...
                      {safeJSONParse(selectedTrade.images, []).length === 0 && <div style={{ fontSize: '12px', color: '#cbd5e1', fontStyle: 'italic' }}>No evidence attached.</div>}
                    </div>
                  </div>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '5px' }}>
                      <div style={{ fontSize: '11px', fontWeight: 'bold', color: '#64748b' }}>EXECUTION CAPTURE</div>
                      <button onClick={handleWebImageUpload} style={{ fontSize: '10px', padding: '2px 8px', background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: '4px', cursor: 'pointer' }}>+ Append</button>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(100px, 1fr))', gap: '8px' }}>
                ...
                <div style={{ background: '#eff6ff', padding: '15px', borderRadius: '12px', border: '1px solid #dbeafe' }}>
                  <div style={{ fontSize: '11px', fontWeight: 'bold', color: '#1e40af', marginBottom: '5px' }}>MACRO NARRATIVE</div>
                  <div style={{ fontSize: '13px', color: '#1e3a8a', fontStyle: 'italic' }}>"{selectedTrade.narrative || "No narrative."}"</div>
                </div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
                <div style={{ background: 'white', padding: '15px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                  <h3 style={{ marginTop: 0, fontSize: '14px', color: '#1e293b', display: 'flex', alignItems: 'center', gap: '8px' }}><AlertOctagon size={18} /> DEVIATIONS</h3>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
                    {MISTAKES_LIST.map(m => (
                      <button key={m} onClick={() => { if (mistakes.includes(m)) setMistakes(mistakes.filter(x => x !== m)); else setMistakes([...mistakes, m]); }} style={{ padding: '4px 8px', borderRadius: '4px', border: '1px solid', fontSize: '11px', cursor: 'pointer', background: mistakes.includes(m) ? '#fee2e2' : 'white', color: mistakes.includes(m) ? '#991b1b' : '#64748b', borderColor: mistakes.includes(m) ? '#fecaca' : '#e2e8f0' }}>{m}</button>
                    ))}
                  </div>
                </div>

                <div style={{ background: 'white', padding: '15px', borderRadius: '12px', border: '1px solid #e2e8f0', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <h3 style={{ marginTop: 0, fontSize: '14px', color: '#1e293b', display: 'flex', alignItems: 'center', gap: '8px' }}><Activity size={18} /> ACTIONABLE DIAGNOSIS</h3>
                  <div style={{ marginBottom: '10px', flex: 1 }}>
                    <label style={{ fontSize: '11px', fontWeight: 'bold', color: '#64748b' }}>POST-MORTEM TAKEAWAYS</label>
                    <textarea value={reviewData.lessons} onChange={e => setReviewData({ ...reviewData, lessons: e.target.value })} style={{ width: '100%', height: '80px', marginTop: '5px', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '12px', fontFamily: 'inherit', resize: 'none' }} placeholder="Post-trade systemic takeaways..." />
                  </div>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: '11px', fontWeight: 'bold', color: '#64748b' }}>ALGORITHMIC ADJUSTMENTS</label>
                    <textarea value={reviewData.action_plan} onChange={e => setReviewData({ ...reviewData, action_plan: e.target.value })} style={{ width: '100%', height: '80px', marginTop: '5px', padding: '8px', borderRadius: '6px', border: '1px solid #fed7aa', fontSize: '12px', fontFamily: 'inherit', background: '#fff7ed', resize: 'none' }} placeholder="System parameters for future iterations..." />
                  </div>
                </div>
              </div>
            </div>
