...
const styles = {
    ...
    statusBadge: (status: string) => {
        ...
        if (status === 'PENDING') { bg = '#fef3c7'; color = '#b45309'; } else if (status === 'PENDING_EXEC' || status === 'ACTIVE' || status === 'FILLED') { bg = '#dcfce7'; color = '#15803d'; }
...
const inputStyle = { width: '100%', padding: '12px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '14px', backgroundColor: 'white', cursor: 'pointer', fontWeight: 500, outline: 'none' };
...
const SectionHeader = ({ title, icon: Icon, isOpen, onClick, sub, colorTheme }: any) => {
    ...
    if (colorTheme === 'orange') { bg = '#fff7ed'; border = '#ffedd5'; iconColor = '#ea580c'; } if (colorTheme === 'blue') { bg = '#eff6ff'; border = '#dbeafe'; iconColor = '#2563eb'; } if (colorTheme === 'purple') { bg = '#faf5ff'; border = '#f3e8ff'; iconColor = '#9338ea'; }
    return (
        <div onClick={onClick} style={styles.header(bg, border) as any}>
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center', fontWeight: 700, color: isOpen ? '#1e293b' : '#64748b', fontSize: '14px' }}>
                <div style={{ padding: '6px', borderRadius: '6px', background: isOpen ? 'white' : 'transparent', boxShadow: isOpen ? '0 1px 2px rgba(0,0,0,0.05)' : 'none' }}><Icon size={18} color={isOpen ? iconColor : '#94a3b8'} /></div>
                <span>{title}</span>
...
const PriceInput = ({ label, value, onChange, color, disabled, placeholder }: any) => (
    <div style={{ flex: 1 }}><label style={{ ...labelStyle, color: color }}>{label}</label>
        <div style={{ position: 'relative' }}><input type="number" step="any" value={value === undefined || value === 0 ? "" : value} disabled={disabled} placeholder={placeholder} onFocus={(e) => e.target.select()} onChange={e => { const val = e.target.value; onChange(val === "" ? 0 : parseFloat(val)); }} style={{ ...inputStyle, borderColor: disabled ? '#f1f5f9' : `${color}60`, background: disabled ? '#f8fafc' : 'white', fontWeight: 'bold', color: color, paddingLeft: '12px' }} /></div>
    </div>
);

const RiskField = ({ label, value, onChange, isOverRisk, color }: any) => (
    <div><label style={{ fontSize: '10px', color: isOverRisk ? '#dc2626' : '#64748b', fontWeight: 700, marginBottom: '4px', display: 'block', textTransform: 'uppercase' }}>{label}</label>
        {onChange ? (<input type="number" step="0.01" value={value} onChange={e => onChange(e.target.value === "" ? 0 : parseFloat(e.target.value))} style={{ width: '100%', fontSize: '18px', fontWeight: 800, border: 'none', borderBottom: isOverRisk ? '2px solid #dc2626' : '1px dashed #cbd5e1', color: isOverRisk ? '#dc2626' : '#0f172a', background: 'transparent', outline: 'none', padding: '4px 0' }} />) : (<div style={{ fontSize: '18px', fontWeight: 800, color: color || '#0f172a', padding: '4px 0', borderBottom: '1px dashed transparent' }}>{value}</div>)}
    </div>
);
...
export default function ScenarioManager({ accountId = 1, prefillData, onClearPrefill }: ScenarioManagerProps) {
    ...
    const loadDynamicData = async () => {
        try {
            const currentWeek = getMondayLocal(new Date()); const baseUrl = "https://mk-project19-1.onrender.com/api";
    ...
    const riskCalc = useMemo(() => {
        if (orderType === "MARKET") { const panicRisk = accountBalance * 0.02; return { safeLots: 999, riskAmt: panicRisk, rr: 0, riskDisplay: "PANIC OVERRIDE (MAX 2% RISK)" }; }
        ...
        let signalMultiplier = 1; if (totalSignalScore < 60) signalMultiplier = 0; else if (totalSignalScore >= 60 && totalSignalScore < 70) signalMultiplier = 0.5;
        ...
        if (portfolioState?.mode === "REDUCED") display += " (Cut 50%)"; if (signalMultiplier === 0.5) display += " (Score Penalty: Cut 50%)";
    ...
    const signalScoreCount = Object.keys(signalScores).length; const isSignalComplete = signalScoreCount === SIGNAL_FEATURES.length;
    ...
    if (!isSignalComplete) validationMessage = `Thiếu Algo Score (${signalScoreCount}/${SIGNAL_FEATURES.length})`; else if (!selectedSetupId) validationMessage = "Chưa chọn Main Model."; else if (!isChecklistComplete) validationMessage = `Thiếu Checklist (${checklistScore}/${requiredTicks})`;
    ...
    const clearForm = () => {
        ...
        setScenarioType(scenarioTypes.length > 0 ? scenarioTypes[0].title : ""); setSignalScores({}); setChecklist({}); setSelectedSetupId(null); setSelectedRiskProfileId(""); setExitStrats([]);
    ...
    const handleSave = async (andExecute = false) => {
        ...
        let uuid = activeScenarioId; const riskSnap = riskCalc ? { profile_id: selectedRiskProfileId, safe_lots: riskCalc.safeLots } : {}; const outlookId = `WEEK-${getMondayLocal(new Date())}`;
        if (!uuid) {
            try { const payload = { input: { ...form, outlook_id: outlookId, account_id: accountId } }; const res = await fetch("https://mk-project19-1.onrender.com/api/scenarios/create/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }); if (!res.ok) throw new Error("API Failure"); const data = await res.json(); uuid = data.uuid; } catch (e) { return toast.error("Lỗi tạo Kịch bản: " + e); }
        }
    ...
    const triggerExecution = async (scenarioData: any) => {
        const priceText = orderType === "MARKET" ? "MARKET PRICE" : scenarioData.entry_price; if (!confirm(`XÁC NHẬN BÓP CÒ?\n${scenarioData.direction} ${scenarioData.pair} @ ${priceText}\nSize: ${scenarioData.volume} Lots`)) return;
        try {
            ...
            toast.success("ĐÃ ĐÓNG DẤU TRIỆN! LÍNH ĐÁNH THUÊ MT5 ĐANG LÊN ĐẠN!"); loadDynamicData();
        } catch (e) { toast.error("Lỗi: " + String(e), { duration: 6000, style: { background: '#ef4444', color: 'white', fontWeight: 'bold' } }); }
    };

    const handleCloseTrade = async (scenario: ScenarioExtended, e: any) => {
        e.stopPropagation(); if (!confirm(`TỔNG TƯ LỆNH XÁC NHẬN RÚT QUÂN LỆNH ${scenario.pair}?`)) return;
        try {
            ...
            toast.success("ĐÃ GỬI LỆNH RÚT QUÂN XUỐNG CHIẾN TRƯỜNG!"); loadDynamicData();
        } catch (e) { toast.error("LỖI LIÊN LẠC: " + String(e)); }
    };

    const handleMarkStatus = async (uuid: string, status: "MISSED" | "CANCELLED", e: any) => {
        e.stopPropagation(); const reason = prompt(`Lý do ${status}?`, "");
        if (reason !== null) { await fetch("https://mk-project19-1.onrender.com/api/scenarios/status/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ uuid, status }) }); toast.success(`Marked as ${status}`); loadDynamicData(); }
    };

    const handleDelete = async (s: ScenarioExtended, e: any) => {
        e.stopPropagation();
        if (['CLOSED', 'FILLED', 'ACTIVE', 'PENDING_EXEC'].includes(s.status)) {
            if (confirm("Lệnh đã cất vào Sổ cái. Ẩn khỏi danh sách Radar này?")) { await fetch("https://mk-project19-1.onrender.com/api/scenarios/status/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ uuid: s.uuid, status: "ARCHIVED" }) }); toast.success("Đã cất cẩn thận!"); loadDynamicData(); if (activeScenarioId === s.uuid) clearForm(); }
        } else {
            if (confirm("Lệnh chưa thực thi. Xóa vĩnh viễn plan này?")) { await fetch("https://mk-project19-1.onrender.com/api/scenarios/delete/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ uuid: s.uuid }) }); toast.success("Deleted."); loadDynamicData(); if (activeScenarioId === s.uuid) clearForm(); }
        }
    };
    ...
    const tradeGrade = useMemo(() => { if (totalSignalScore >= 85) return { grade: "A+", color: "#16a34a", text: "FULL ALLOCATION" }; if (totalSignalScore >= 70) return { grade: "B", color: "#2563eb", text: "STANDARD ALLOCATION" }; if (totalSignalScore >= 60) return { grade: "C", color: "#d97706", text: "REDUCED EXPOSURE (-50%)" }; return { grade: "F", color: "#dc2626", text: "EXECUTION BLOCKED" }; }, [totalSignalScore]);

    const isRiskOn = outlookData.sentiment === 'Risk-On'; const isRiskOff = outlookData.sentiment === 'Risk-Off';
    const topBarBg = isRiskOn ? '#dcfce7' : (isRiskOff ? '#fee2e2' : 'white'); const sentimentColor = isRiskOn ? '#166534' : (isRiskOff ? '#991b1b' : '#0f172a');
    const biasStr = outlookData.bias || ""; const isBiasBullish = biasStr.includes('BULLISH') || biasStr.includes('BUY') || biasStr.includes('LONG'); const isBiasBearish = biasStr.includes('BEARISH') || biasStr.includes('SELL') || biasStr.includes('SHORT');
            ...
            <div style={{ ...styles.column, width: '320px' }}>
                ...
                <div style={{ flex: 1, overflowY: "auto", padding: '12px' }}>
                    {scenarios.filter(s => !['MISSED', 'CANCELLED', 'ARCHIVED'].includes(s.status)).slice(0, 20).map(s => {
                        const isPending = s.status === 'PENDING'; const isLive = ['PENDING_EXEC', 'ACTIVE', 'FILLED'].includes(s.status); const isClosed = ['CLOSED', 'MISSED', 'CANCELLED', 'ARCHIVED'].includes(s.status); const isSelected = s.uuid === activeScenarioId; let scoreVal = 0; try { const c = JSON.parse(s.pre_trade_checklist || "{}"); scoreVal = c.score || 0; } catch { }
                        return (
                            <div key={s.uuid} onClick={() => loadScenario(s)} style={styles.scenarioCard(isSelected, s.direction as any)}>
                                ...
                                {/* HIỂN THỊ ACTIVE EXPOSURE (LIVE PNL VÀ VOLUME) NGAY TẠI ĐÂY */}
                                {isLive && (
                                    <div style={{ background: '#f8fafc', padding: '6px 10px', borderRadius: '6px', marginTop: '8px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', border: '1px solid #cbd5e1' }}>
                                        <div style={{ fontSize: '11px', color: '#0f172a', fontWeight: '900' }}>VOL: {s.volume}</div>
                                        <div style={{ fontSize: '14px', fontWeight: '900', color: (s.pnl || 0) >= 0 ? '#16a34a' : '#dc2626' }}>
                                            {(s.pnl || 0) >= 0 ? '+' : ''}{Number(s.pnl || 0).toFixed(2)}$
                                        </div>
                                    </div>
                                )}

                                <div style={{ display: 'flex', gap: '5px', paddingTop: '8px', borderTop: '1px dashed #e2e8f0', marginTop: '8px' }}>
                                    {isPending && (<><button onClick={(e) => { e.stopPropagation(); loadScenario(s); triggerExecution(s); }} style={styles.actionBtn('white', '#0f172a')} title="Execute"><Play size={14} /></button><button onClick={(e) => { e.stopPropagation(); loadScenario(s); }} style={styles.actionBtn('#1d4ed8', '#eff6ff')} title="Edit"><Edit3 size={14} /></button><button onClick={(e) => handleMarkStatus(s.uuid, "MISSED", e)} style={styles.actionBtn('#d97706', '#fffbeb')} title="Missed"><EyeOff size={14} /></button><button onClick={(e) => handleMarkStatus(s.uuid, "CANCELLED", e)} style={styles.actionBtn('#475569', '#f1f5f9')} title="Cancel"><Ban size={14} /></button></>)}
                                    {isLive && (<button onClick={(e) => handleCloseTrade(s, e)} style={styles.actionBtn('white', '#ef4444')} title="Close Position"><StopCircle size={14} /></button>)}
            ...
            <div style={{ ...styles.column, flex: 1 }}>
                <div style={{ padding: '15px 25px', background: topBarBg, borderBottom: '1px solid #e2e8f0', display: 'flex', gap: '25px', alignItems: 'center', justifyContent: 'space-between', transition: 'all 0.3s' }}>
                    <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                        <select value={form.pair} onChange={e => setForm({ ...form, pair: e.target.value })} style={{ fontSize: '18px', fontWeight: 'bold', padding: '10px 15px', borderRadius: '10px', border: '1px solid #cbd5e1', width: '140px', outline: 'none' }}>{ALL_PAIRS.map(p => <option key={p} value={p}>{p}</option>)}</select>
                        <div style={{ display: 'flex', gap: '10px' }}><button onClick={() => setForm({ ...form, direction: "BUY" })} style={{ width: '90px', padding: '10px 0', borderRadius: '8px', border: form.direction === 'BUY' ? '2px solid #16a34a' : '1px solid #cbd5e1', background: form.direction === 'BUY' ? '#dcfce7' : 'white', color: form.direction === 'BUY' ? '#15803d' : '#64748b', fontWeight: '800', cursor: 'pointer', boxShadow: form.direction === 'BUY' ? '0 4px 6px -1px rgba(22, 163, 74, 0.2)' : 'none' }}>LONG</button><button onClick={() => setForm({ ...form, direction: "SELL" })} style={{ width: '90px', padding: '10px 0', borderRadius: '8px', border: form.direction === 'SELL' ? '2px solid #dc2626' : '1px solid #cbd5e1', background: form.direction === 'SELL' ? '#fee2e2' : 'white', color: form.direction === 'SELL' ? '#991b1b' : '#64748b', fontWeight: '800', cursor: 'pointer', boxShadow: form.direction === 'SELL' ? '0 4px 6px -1px rgba(220, 38, 38, 0.2)' : 'none' }}>SHORT</button></div>
                    </div>
                    <div style={{ display: 'flex', gap: '20px', alignItems: 'center', flexWrap: 'wrap' }}>
                        <div style={{ textAlign: 'center' }}><div style={{ fontSize: '10px', fontWeight: 'bold', color: '#64748b', textTransform: 'uppercase' }}>Sentiment</div><div style={{ fontSize: '16px', fontWeight: 900, color: sentimentColor }}>{outlookData.sentiment}</div></div>
                        <div style={{ textAlign: 'center' }}><div style={{ fontSize: '10px', fontWeight: 'bold', color: '#64748b', textTransform: 'uppercase' }}>My Mood</div><select value={myMood} onChange={e => setMyMood(e.target.value)} style={{ padding: '4px 8px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '13px', fontWeight: 'bold', outline: 'none', color: '#059669', background: '#f0fdf4' }}><option value="FOCUSED">🧘‍♂️ FOCUSED</option><option value="FOMO">🔥 FOMO</option><option value="REVENGE">🤬 REVENGE</option><option value="TIRED">🥱 TIRED</option></select></div>
                        <div style={{ textAlign: 'center', borderRight: '1px solid #cbd5e1', paddingRight: '20px' }}><div style={{ fontSize: '10px', fontWeight: 'bold', color: '#64748b', textTransform: 'uppercase' }}>Weekly Bias</div><div style={{ fontSize: '16px', fontWeight: 900, color: biasColor, textTransform: 'uppercase' }}>{outlookData.bias}</div></div>
                ...
                <div style={{ flex: 1, overflowY: 'auto' }}>
                    {biasCheck.status !== "ALIGNED" && (<div style={{ background: biasCheck.color === '#dc2626' ? '#fef2f2' : '#f8fafc', padding: '10px 25px', borderBottom: `1px dashed ${biasCheck.color}`, color: biasCheck.color, fontWeight: 'bold', fontSize: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}><AlertTriangle size={16} /> LƯU Ý TỪ HẢI ĐĂNG: {biasCheck.text}</div>)}
                    ...
                    {expandedSection === "CONTEXT" && (
                        <div style={{ padding: '25px', background: '#fff7ed' }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '20px', marginBottom: '20px' }}><div><label style={labelStyle}>HTF Trend</label><select style={inputStyle} value={context.htf_trend} onChange={e => setContext({ ...context, htf_trend: e.target.value })}><option>Bullish</option><option>Bearish</option><option>Range</option></select></div><div><label style={labelStyle}>Market Phase</label><select style={inputStyle} value={context.market_phase} onChange={e => setContext({ ...context, market_phase: e.target.value })}><option>Expansion</option><option>Retracement</option><option>Accumulation</option><option>Distribution</option></select></div><div><label style={labelStyle}>Dealing Range</label><select style={inputStyle} value={context.dealing_range} onChange={e => setContext({ ...context, dealing_range: e.target.value })}><option>Discount</option><option>Premium</option><option>Equilibrium</option></select></div></div>
                            <div style={{ marginBottom: '15px' }}><div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '8px' }}><label style={{ ...labelStyle, marginBottom: 0 }}>Narrative & Context</label>{!isAddingTag && (<button onClick={() => setIsAddingTag(true)} style={{ fontSize: '11px', padding: '4px 8px', borderRadius: '6px', border: '1px dashed #94a3b8', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, color: '#64748b', fontWeight: 600 }}><Tag size={12} /> + Add Tag</button>)}</div>{isAddingTag && (<div style={{ padding: 10, background: 'white', borderRadius: 8, border: '1px solid #fdba74', marginBottom: 10 }}><div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>{tagsToDisplay.map((t: any) => (<button key={t.id} onClick={() => setContext({ ...context, narrative: (context.narrative + ` [#${t.title}] `).trim() })} style={{ padding: '4px 10px', borderRadius: '12px', border: '1px solid #e2e8f0', background: '#fff7ed', fontSize: '11px', color: '#c2410c', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '4px' }}>{t.title}</button>))}</div><button onClick={() => setIsAddingTag(false)} style={{ marginTop: 8, fontSize: '11px', color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Close</button></div>)}</div>
                    ...
                    {expandedSection === "SCENARIO" && (
                        <div style={{ padding: '25px' }}>
                            <div style={{ marginBottom: '25px', background: '#f8fafc', padding: '20px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', borderBottom: '1px dashed #cbd5e1', paddingBottom: '10px' }}><span style={{ fontSize: '14px', fontWeight: 800, color: '#1e293b' }}>Algorithmic Assessment</span><div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}><span style={{ fontSize: '24px', fontWeight: 900, color: tradeGrade.color }}>{totalSignalScore}</span><div style={{ display: 'flex', flexDirection: 'column' }}><span style={{ fontSize: '10px', color: '#64748b', textTransform: 'uppercase' }}>/100 Points</span><span style={{ fontSize: '11px', fontWeight: 800, color: tradeGrade.color }}>{tradeGrade.grade} GRADE</span></div></div></div>
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>{SIGNAL_FEATURES.map((feature) => (<div key={feature.id}><label style={{ fontSize: '11px', fontWeight: 'bold', color: '#475569', marginBottom: '6px', display: 'block' }}>{feature.label}</label><div style={{ display: 'flex', gap: '6px' }}>{feature.options.map((opt) => { const isSelected = signalScores[feature.id] === opt.val; return (<button key={opt.label} onClick={() => setSignalScores(prev => ({ ...prev, [feature.id]: opt.val }))} style={{ flex: 1, padding: '6px 4px', fontSize: '10px', fontWeight: 600, borderRadius: '6px', cursor: 'pointer', transition: 'all 0.2s', background: isSelected ? '#3b82f6' : 'white', color: isSelected ? 'white' : '#64748b', border: isSelected ? '1px solid #2563eb' : '1px solid #cbd5e1', boxShadow: isSelected ? '0 2px 4px rgba(59, 130, 246, 0.3)' : 'none' }}>{opt.label} ({opt.val})</button>) })}</div></div>))}</div>
                            </div>
                            <div style={{ marginBottom: '25px' }}><label style={labelStyle}>Scenario Type</label><select style={inputStyle} value={scenarioType} onChange={e => setScenarioType(e.target.value)}>{scenarioTypes.length > 0 ? (scenarioTypes.map(t => <option key={t.id} value={t.title}>{t.title}</option>)) : (<option>Loading or Empty...</option>)}</select></div>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
                                <div><label style={labelStyle}>Setup Conditions (Dynamic)</label><div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px', border: '1px solid #e2e8f0', maxHeight: '300px', overflowY: 'auto' }}>{(!selectedSetupId) ? (<div style={{ fontSize: '13px', color: '#94a3b8', fontStyle: 'italic', textAlign: 'center', padding: '20px 0' }}>Vui lòng chọn 'Main Model' bên cạnh để tải Checklist.</div>) : checklistItems.length === 0 ? (<div style={{ fontSize: '13px', color: '#94a3b8', fontStyle: 'italic', textAlign: 'center', padding: '20px 0' }}>Setup này chưa có Checklist cấu hình sẵn.</div>) : (checklistItems.map((item, i) => (<label key={i} style={{ display: 'flex', gap: '10px', fontSize: '13px', marginBottom: '10px', cursor: 'pointer', color: checklist[item] ? '#166534' : '#475569', fontWeight: 500 }}><input type="checkbox" checked={checklist[item] || false} onChange={e => setChecklist({ ...checklist, [item]: e.target.checked })} style={{ transform: 'scale(1.2)' }} /> {item}</label>)))}</div></div>
                                <div><label style={labelStyle}>Main Model</label><select value={selectedSetupId || ""} onChange={e => setSelectedSetupId(Number(e.target.value))} style={inputStyle}><option value="">-- Chọn Setup --</option>{setupLibrary.map((s: any) => <option key={s.id} value={s.id}>{s.title}</option>)}</select><div style={{ marginTop: '25px', padding: '20px', background: isStage2Valid ? '#f0fdf4' : '#fef2f2', borderRadius: '12px', border: isStage2Valid ? '1px solid #bbf7d0' : '1px solid #fecaca', display: 'flex', alignItems: 'center', gap: '15px' }}>{isStage2Valid ? <CheckCircle2 size={24} color="#16a34a" /> : <Lock size={24} color="#dc2626" />}<div style={{ display: 'flex', flexDirection: 'column' }}><span style={{ fontSize: '14px', fontWeight: 700, color: isStage2Valid ? '#166534' : '#991b1b' }}>{validationMessage}</span></div></div></div>
                            </div>
                    ...
                    {expandedSection === "EXEC" && (
                        <div style={{ padding: '25px', opacity: isStage2Valid ? 1 : 0.6, pointerEvents: isStage2Valid ? 'auto' : 'none' }}>
                            {isFrozen && (<div style={{ background: '#fef2f2', border: '1px dashed #ef4444', color: '#b91c1c', padding: '12px', borderRadius: '8px', marginBottom: '20px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px' }}><Snowflake size={18} /> LỆNH PHẠT TỪ CEO: TÀI KHOẢN ĐÓNG BĂNG. Vốn cấp phát = 0. Không thể vào lệnh.</div>)}
                            {portfolioState?.account_status === "CLAMPED" && (<div style={{ background: '#fffbeb', border: '1px dashed #f59e0b', color: '#b45309', padding: '12px', borderRadius: '8px', marginBottom: '20px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px' }}><AlertTriangle size={18} /> CEO CLAMP: Hãm hưng phấn! Hệ thống tự động CẮT 50% VỐN CẤP PHÁT để ép bạn giảm Volume.</div>)}
                            {portfolioState?.mode === "REDUCED" && !isFrozen && (<div style={{ background: '#fef3c7', border: '1px dashed #f59e0b', color: '#b45309', padding: '12px', borderRadius: '8px', marginBottom: '20px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px' }}><ShieldAlert size={18} /> CEO MODE: REDUCED (Hệ thống tổng cắt giảm 50% rủi ro)</div>)}
                            {isScoreTooLow && (<div style={{ background: '#fef2f2', border: '1px dashed #ef4444', color: '#b91c1c', padding: '12px', borderRadius: '8px', marginBottom: '20px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px' }}><Ban size={18} /> SUB-OPTIMAL SIGNAL (Score &lt; 60): Fails quantitative threshold. Execution prohibited.</div>)}

                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '25px', marginBottom: '30px' }}>
                                <div style={{ flex: 1 }}><label style={labelStyle}>Order Type</label><select value={orderType} onChange={e => setOrderType(e.target.value as any)} style={inputStyle}><option value="LIMIT">LIMIT</option><option value="STOP">STOP</option><option value="MARKET">MARKET</option></select></div>
                            ...
                            <div style={{ background: '#faf5ff', padding: '25px', borderRadius: '16px', border: '1px solid #f3e8ff', marginBottom: '30px', boxShadow: 'inset 0 2px 4px rgba(0,0,0,0.02)' }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}><h4 style={{ margin: 0, fontSize: '13px', color: '#7e22ce', fontWeight: 800, display: 'flex', gap: '6px', alignItems: 'center' }}><Activity size={16} /> DYNAMIC RISK ENGINE (Capital Base: ${accountBalance.toLocaleString(undefined, { maximumFractionDigits: 0 })})</h4><div style={{ textAlign: 'right' }}><span style={{ fontSize: '13px', fontWeight: 'bold', color: '#6b21a8' }}>Risk: {riskCalc?.riskDisplay} | Projected RR: {riskCalc ? riskCalc.rr.toFixed(2) : '0.00'}R</span></div></div>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '30px', alignItems: 'center' }}>
                                    ...
                                    <div><label style={labelStyle}>RISK PROFILE</label><select value={selectedRiskProfileId} onChange={e => setSelectedRiskProfileId(e.target.value)} style={{ ...inputStyle, border: '1px solid #d8b4fe' }}><option value="">-- Manual (1%) --</option>{riskProfiles.map((p: any) => <option key={p.id} value={p.id}>{p.title}</option>)}</select></div>
                                </div>
                            </div>

                            <div style={{ marginBottom: '30px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}><div style={{ display: 'flex', gap: 8, alignItems: 'center' }}><ShieldCheck size={18} color="#475569" /><label style={{ ...labelStyle, marginBottom: 0 }}>4. EXIT STRATEGY (Required)</label></div>{!isAddingExit && (<button onClick={() => setIsAddingExit(true)} style={{ fontSize: '11px', padding: '4px 8px', borderRadius: '6px', border: '1px dashed #94a3b8', background: 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, color: '#64748b', fontWeight: 600 }}><ListPlus size={14} /> Add Strat</button>)}</div>
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: isAddingExit ? 10 : 0 }}>{exitStrats.map(strat => (<div key={strat} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '20px', fontSize: '12px', fontWeight: 600, color: '#166534' }}>{strat}<button onClick={() => setExitStrats(p => p.filter(s => s !== strat))} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', padding: 0, color: '#15803d' }}><X size={14} /></button></div>))}</div>
                                {isAddingExit && (<div style={{ padding: 10, background: '#f8fafc', borderRadius: 8, border: '1px solid #e2e8f0' }}><div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>{exitLibrary.filter(ex => !exitStrats.includes(ex.title)).map(ex => (<button key={ex.id} onClick={() => { setExitStrats(p => [...p, ex.title]); setIsAddingExit(false); }} style={{ padding: '6px 12px', background: 'white', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '12px', cursor: 'pointer', fontWeight: 500, color: '#475569', boxShadow: '0 1px 2px rgba(0,0,0,0.05)' }}>{ex.title}</button>))}</div><button onClick={() => setIsAddingExit(false)} style={{ marginTop: 10, fontSize: '11px', color: '#ef4444', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Cancel</button></div>)}
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: '20px' }}>
                                ...
                                <button onClick={() => handleSave(true)} disabled={disableExecution} style={styles.bigButton('execute', disableExecution) as any}><Send size={20} /> {isAlreadyExecuted ? "LỆNH ĐÃ THỰC THI" : "EXECUTE TRADE"}</button>
                            </div>
                        </div>
                    )}
                </div>
