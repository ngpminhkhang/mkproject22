...
export default function Library() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Đúc vũ khí thành công!");
        setEditingItem(null);
        loadItems();
      } else {
        const errText = await res.text();
        toast.error("BÁO CÁO LỖI: " + errText.substring(0, 80));
      }
    } catch (e) { toast.error("Kẹt API: " + e); }
  };
  ...
  return (
    <div style={{ padding: '20px', fontFamily: "'Inter', sans-serif", height: 'calc(100vh - 80px)', overflowY: 'auto', backgroundColor: '#f1f5f9' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', background: 'white', padding: '15px 20px', borderRadius: '12px', border: '1px solid #e2e8f0', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
        <h2 style={{ margin: 0, color: '#1e293b', fontSize: '20px', fontWeight: 900 }}>KHO VŨ KHÍ (SYSTEM LIBRARY)</h2>
      ...
      {editingItem && (
        <div style={{ background: 'white', padding: '20px', borderRadius: '12px', border: '2px solid #3b82f6', marginBottom: '20px', boxShadow: '0 10px 15px -3px rgba(59,130,246,0.2)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
            <h3 style={{ margin: 0, color: '#1e40af' }}>{editingItem.id ? "SỬA VŨ KHÍ" : "ĐÚC VŨ KHÍ MỚI"}</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '15px' }}>
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 'bold', color: '#64748b', marginBottom: '5px' }}>TÊN VŨ KHÍ</label>
              <input value={editingItem.title} onChange={e => setEditingItem({ ...editingItem, title: e.target.value })} style={{ width: '100%', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '14px', outline: 'none' }} placeholder="VD: M15 BOS & Sweep..." />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 'bold', color: '#64748b', marginBottom: '5px' }}>PHÂN LOẠI</label>
            ...
            <div style={{ gridColumn: '1 / -1' }}>
              <label style={{ display: 'block', fontSize: '12px', fontWeight: 'bold', color: '#64748b', marginBottom: '5px' }}>CẤU HÌNH LÕI (JSON Dành Cho Risk Model/Kelly)</label>
              <textarea value={editingItem.configuration} onChange={e => setEditingItem({ ...editingItem, configuration: e.target.value })} style={{ width: '100%', height: '80px', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1', fontSize: '14px', outline: 'none', fontFamily: 'monospace' }} placeholder='VD: {"type": "KELLY", "fraction": 0.5}' />
            </div>
          </div>
          <button onClick={handleSave} style={{ background: '#2563eb', color: 'white', border: 'none', padding: '10px 20px', borderRadius: '6px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer' }}><Save size={16}/> LƯU VÀO KHO</button>
        </div>
      )}
        ...
        {CATEGORIES.map(cat => (
          <div key={cat.id} style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '15px', display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '300px', overflowY: 'auto' }}>
              ...
              {items.filter(i => i.category === cat.id).length === 0 && <div style={{ fontSize: '12px', color: '#94a3b8', fontStyle: 'italic', textAlign: 'center' }}>Khoang chứa trống.</div>}
            </div>
          </div>
        ))}
      </div>
