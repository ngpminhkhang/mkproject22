...
export default function WeeklyForm({ accountId = 1, onSendToScenario }: { accountId?: number, onSendToScenario?: (signal: any) => void }) {
    ...
    const handleSave = async () => {
        // CÚ ĐẤM VÀO ĐÚNG CỔNG API MỚI!
        const payload = {
            week_start: currentWeek,
            market_sentiment: global.risk,
            weekly_bias: finalBias,
        ...
        try {
            ...
            if (res.ok) toast.success("Ngọn Hải Đăng đã thắp sáng! Dữ liệu đã truyền xuống Két Sắt.");
            else toast.error("Synchronization Failed. Cổng API từ chối.");
        } catch (e) { toast.error("API Error: " + String(e)); }
    };
