...
export default function WeeklyForm({ accountId = 1 }: { accountId?: number }) {
    ...
    // ĐỘNG CƠ HÚT DỮ LIỆU TỪ ĐÁM MÂY
    const loadData = async () => {
        try {
            ...
            console.error(e);
            toast.error("Mất kết nối với Đám mây");
        }
    };
        ...
        <div style={{ padding: '20px', background: '#f8fafc', minHeight: '100vh', fontFamily: 'Segoe UI, sans-serif' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                    <input type="date" style={{ padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1', fontWeight: 'bold' }} value={date} onChange={handleDateChange} />
                    <button onClick={() => handleReset(true)} style={{ padding: '8px 15px', borderRadius: '6px', border: '1px solid #cbd5e1', background: 'white', cursor: 'pointer', display: 'flex', gap: '5px', alignItems: 'center' }}>
                        <RotateCcw size={18} /> Reset
                    </button>
                    <button onClick={handleSave} style={{ padding: '8px 15px', borderRadius: '6px', border: 'none', background: '#2563eb', color: 'white', fontWeight: 'bold', cursor: 'pointer', display: 'flex', gap: '5px', alignItems: 'center' }}>
                        <Save size={18} /> SYNC TO NODE
                    </button>
                </div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1.3fr 1fr', gap: '25px', alignItems: 'start' }}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    <div style={cardStyle}>
                        <h3 style={{ ...headerStyle, color: 'var(--primary)' }}><Globe size={18} /> 1. Global Regime & Macro</h3>
                        <div style={{ background: '#f8fafc', padding: '15px', borderRadius: '8px', marginBottom: '15px', border: '1px solid #e2e8f0', textAlign: 'center' }}>
                            ...
                            <select value={global.risk} onChange={e => setGlobal({ ...global, risk: e.target.value as any })} style={{ width: '100%', color: getColor(global.risk), fontWeight: '900', fontSize: '1.4rem', textAlign: 'center', height: '50px', border: `2px solid ${getColor(global.risk)}`, borderRadius: '8px', background: 'white', cursor: 'pointer' }}>
                                <option>Risk-On</option><option>Mixed</option><option>Risk-Off</option>
                            </select>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '15px' }}>
                            <div><label style={{ fontSize: '0.8rem', color: '#64748b' }}>Liquidity</label><select style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1', color: getColor(global.liquidity), fontWeight: 'bold' }} value={global.liquidity} onChange={e => setGlobal({ ...global, liquidity: e.target.value as any })}><option>Tightening</option><option>Neutral</option><option>Easing</option></select></div>
                            <div><label style={{ fontSize: '0.8rem', color: '#64748b' }}>Volatility</label><select style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1', color: getColor(global.volatility), fontWeight: 'bold' }} value={global.volatility} onChange={e => setGlobal({ ...global, volatility: e.target.value as any })}><option>Normal</option><option>Elevated</option><option>Extreme</option></select></div>
                            <div><label style={{ fontSize: '0.8rem', color: '#64748b' }}>Macro Phase</label><select style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1', color: getColor(global.macro_phase), fontWeight: 'bold' }} value={global.macro_phase} onChange={e => setGlobal({ ...global, macro_phase: e.target.value as any })}><option>Expansion</option><option>Slowdown</option><option>Recession</option><option>Recovery</option></select></div>
                            <div style={{ gridColumn: '1/-1', borderTop: '1px dashed #e2e8f0', margin: '5px 0' }}></div>
                            <div><label style={{ fontSize: '0.8rem', color: '#64748b' }}>USD Struct</label><select style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1', color: getColor(macro.usd_structural), fontWeight: 'bold' }} value={macro.usd_structural} onChange={e => setMacro({ ...macro, usd_structural: e.target.value as any })}><option>Bullish</option><option>Neutral</option><option>Bearish</option></select></div>
                            <div><label style={{ fontSize: '0.8rem', color: '#64748b' }}>Yield Bias</label><select style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1', color: getColor(macro.yield_bias), fontWeight: 'bold' }} value={macro.yield_bias} onChange={e => setMacro({ ...macro, yield_bias: e.target.value as any })}><option>USD Advantage</option><option>Neutral</option><option>Non-USD Adv</option></select></div>
                            <div><label style={{ fontSize: '0.8rem', color: '#64748b' }}>Commodity</label><select style={{ width: '100%', padding: '8px', borderRadius: '6px', border: '1px solid #cbd5e1', color: getColor(macro.commodity), fontWeight: 'bold' }} value={macro.commodity} onChange={e => setMacro({ ...macro, commodity: e.target.value as any })}><option>Support FX</option><option>Neutral</option><option>Weighs FX</option></select></div>
                        </div>
                    </div>
                        ...
                        <div style={{ overflowX: 'auto' }}>
                            ...
                            {BANKS.map(id => (
                                <div key={id} style={{ display: 'grid', gridTemplateColumns: '40px 1fr 1fr 1fr', gap: '8px', alignItems: 'center', marginBottom: '6px', paddingBottom: '6px', borderBottom: '1px dashed #f8fafc' }}>
                                    ...
                                    <select style={{ padding: '2px', borderRadius: '4px', border: '1px solid #e2e8f0', fontSize: '0.75rem', height: 'auto', color: getColor(banks[id].stance), fontWeight: 'bold' }} value={banks[id].stance} onChange={e => updateBank(id, 'stance', e.target.value)}><option>Tightening</option><option>Hold</option><option>Easing</option><option>Neutral</option></select>
                                    <select style={{ padding: '2px', borderRadius: '4px', border: '1px solid #e2e8f0', fontSize: '0.75rem', height: 'auto', color: getColor(banks[id].tone), fontWeight: 'bold' }} value={banks[id].tone} onChange={e => updateBank(id, 'tone', e.target.value)}><option>Hawkish</option><option>Neutral</option><option>Dovish</option></select>
                                    <select style={{ padding: '2px', borderRadius: '4px', border: '1px solid #e2e8f0', fontSize: '0.75rem', height: 'auto', color: getColor(banks[id].gap), fontWeight: 'bold' }} value={banks[id].gap} onChange={e => updateBank(id, 'gap', e.target.value)}><option>CB Ahead</option><option>Aligned</option><option>Market Ahead</option></select>
                                </div>
                            ))}
                        </div>
                        <textarea style={{ width: '100%', marginTop: '10px', height: '60px', fontSize: '0.85rem', padding: '8px', borderRadius: '6px', border: '1px solid #e2e8f0' }} placeholder="Ghi chú: Sự phân kỳ chính sách (Divergence) ở đâu?..." value={faNotes} onChange={e => setFaNotes(e.target.value)} />
                    </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px 20px' }}>
                            {CURRENCIES.map(curr => (
                                <div key={curr} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                    ...
                                    <input type="range" min="-3" max="3" step="1" value={scores[curr]} onChange={e => setScores({ ...scores, [curr]: Number(e.target.value) })} style={{ flex: 1, margin: '0 10px', height: '6px', cursor: 'pointer' }} />
                    ...
                    <div style={{ ...cardStyle, borderLeft: '4px solid var(--primary)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                            ...
                            <button onClick={handleAutoPlan} title="Auto-Write Plan based on Matrix" style={{ color: '#7c3aed', background: '#f3e8ff', border: 'none', borderRadius: '12px', padding: '4px 8px', fontSize: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '5px' }}>
                        ...
                        <textarea style={{ width: '100%', height: '150px', fontFamily: 'monospace', fontSize: '0.85rem', padding: '10px', borderRadius: '6px', border: '1px solid #cbd5e1' }} placeholder="Bấm nút đũa thần để tự động viết kế hoạch..." value={scriptPlan} onChange={e => setScriptPlan(e.target.value)} />
                    </div>
                </div>
            </div>
        </div>
