...
export default function WeeklyReviewHub({ accountId = 1 }: { accountId?: number }) {
    ...
    const [outlookSnapshot, setOutlookSnapshot] = useState<{ bias: string, plan: string, technical: string, matrix: any[] }>({ bias: 'NEUTRAL', plan: '', technical: '', matrix: [] });
    ...
    const loadData = async () => {
        ...
        try {
            ...
            // 3. TẢI OUTLOOK & TECHNICAL PLAN (Đã đấu nối ống nước chuẩn chỉnh)
    ...
    const handleSaveReview = async () => {
        try {
            ...
            // ĐỘI XE 2: CHỞ TECHNICAL PLAN VÀO OUTLOOK HẦM CHỨA
            const outlookPayload = {
                week_start_date: currentWeek,
                account_id: accountId,
                fa_bias: { technical_plan: outlookSnapshot.technical, planned: outlookSnapshot.matrix },
            ...
            toast.success("Hồ sơ thẩm vấn đã đóng mộc đỏ!");
        } catch (e) { 
